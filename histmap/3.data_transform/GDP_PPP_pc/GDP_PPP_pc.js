global.GDP_PPP_pc = class {
	...
	static intermediate_gdp_ppp_scaled_to_national = GDP_PPP.intermediate_scaled_to_national;
	...
	static async A_generateGDP_pcRasters (arg0_options) {
		...
		let gdp_obj = GDP_PPP.getGDP_PPPObject();
		
		if (!fs.existsSync(out_dir)) fs.mkdirSync(out_dir, { recursive: true });
		
		for (let i = 0; i < hyde_years.length; i++) {
			let local_output_path = `${this.input_gdp_pc_folder}GDP_PPP_pc_${hyde_years[i]}.png`;
		...
		return GeoPNG.processTimeseriesParallel({
			...
			items: target_years,
			name: "GDP_PPP_pc A_generateGDP_pcRasters (Uniform Historical Target)",
			handler: async (year) => {
				let local_output_path = `${this.input_gdp_pc_folder}GDP_PPP_pc_${year}.png`;
	...
	static async B_trainGDP_PPP_pcModels (arg0_options) {
		...
		await Statistics.trainOLSModelsParallel(years, (year) => {
			...
			return {
				...
				output_file_path: `${this.intermediate_ols_folder}OLS_GDP_PPP_pc_${year}.json`,
				target_file_path: `${this.input_gdp_pc_folder}GDP_PPP_pc_${year}.png`,
				target_format: "float32"
			};
		}, {
			concurrency: options.concurrency || 8,
			name: "GDP_PPP_pc 2nd-pass OLS Training"
		});
		
		// 2. Identify invalid models (all coefficients are 0) and temporarily rename them so they aren't averaged
		let invalid_years = [];
		for (let i = 0; i < years.length; i++) {
			let file_path = `${this.intermediate_ols_folder}OLS_GDP_PPP_pc_${years[i]}.json`;
			if (fs.existsSync(file_path)) {
				...
				if (sum_coeffs === 0) {
					...
					fs.renameSync(file_path, `${this.intermediate_ols_folder}INVALID_OLS_GDP_PPP_pc_${years[i]}.json`);
				}
			}
		}
		
		// 3. Compute geometric average of valid OLS models
		let geomean_data = await Statistics.geomeanOLSModels(this.intermediate_ols_folder, "OLS_GDP_PPP_pc_");
		
		// 4. Restore and overwrite invalid models with the geomean
		for (let i = 0; i < invalid_years.length; i++) {
			let invalid_path = `${this.intermediate_ols_folder}INVALID_OLS_GDP_PPP_pc_${invalid_years[i]}.json`;
			let target_path = `${this.intermediate_ols_folder}OLS_GDP_PPP_pc_${invalid_years[i]}.json`;
	...
	static async C_generateOLS_GDP_PPP_pcRasters (arg0_options) {
		//Convert from parameters
		let options = (arg0_options) ? arg0_options : {};
		...
		for (let i = 0; i < hyde_years.length; i++) {
			let local_input_path = `${this.intermediate_ols_folder}OLS_GDP_PPP_pc_${hyde_years[i]}.json`;
			let local_output_path = `${this.intermediate_ols_rasters_folder}OLS_GDP_PPP_pc_${hyde_years[i]}.png`;
		...
		return Statistics.generateOLSRastersParallel(target_years, (year) => {
			...
			return {
				covariates_map: covariates_map,
				model_obj: `${this.intermediate_ols_folder}OLS_GDP_PPP_pc_${year}.json`,
				options: {
					format: "float32"
				},
				output_file_path: `${this.intermediate_ols_rasters_folder}OLS_GDP_PPP_pc_${year}.png`
			};
		}, {
			concurrency: options.concurrency || 8,
			name: "GDP_PPP_pc OLS Raster Generation"
		});
	}
	
	static async D_normaliseGDP_PPP_pcRasters (arg0_options) {
		let options = (arg0_options) ? arg0_options : {};
		let years = landuse_HYDE.sorted_hyde_years;
		...
		console.log(`[GDP_PPP_pc] Running fast sequential pre-pass for Step D thresholds...`);
		let final_year = years[years.length - 1];
		let first_pass_path = `${this.input_gdp_pc_folder}GDP_PPP_pc_${final_year}.png`;
		...
		await GeoPNG.processTimeseriesParallel({
			concurrency: options.concurrency || 8,
			items: years,
			name: "GDP_PPP_pc D_normaliseGDP_PPP_pcRasters",
			handler: async (year) => {
				if (!thresholds[year]) return null;
				let prev_max = thresholds[years[Math.max(0, years.indexOf(year) - 1)]] || 0;
				
				let first_pass_path = `${this.input_gdp_pc_folder}GDP_PPP_pc_${year}.png`;
				let second_pass_path = `${this.intermediate_ols_rasters_folder}OLS_GDP_PPP_pc_${year}.png`;
				let pop_path = `${population_Stadester.input_popc_folder}stadester_population_${year}.png`;
				let output_path = `${this.intermediate_pc_estimates_folder}GDP_PPP_pc_${year}.png`;
	...
	static async G_interpolateToSEDAC (arg0_options) {
		...
		//Declare local instance variables
		let hyde_years = landuse_HYDE.sorted_hyde_years;
		let out_dir = (typeof path !== "undefined") ? path.resolve(this.intermediate_gdp_ppp_interpolated) : this.intermediate_gdp_ppp_interpolated;
		let overwrite = (options.overwrite !== undefined) ? options.overwrite : true;
		...
		for (let i = 0; i < hyde_years.length; i++) {
			let local_output_path = `${this.intermediate_gdp_ppp_interpolated}GDP_PPP_${hyde_years[i]}.png`;
			if (overwrite || !fs.existsSync(local_output_path))
				target_years.push(hyde_years[i]);
		}
		...
		return GeoPNG.processTimeseriesParallel({
			concurrency: options.concurrency || 8,
			items: target_years,
			name: "GDP_PPP_pc G_interpolateToSEDAC",
	...
	static async H_scaleGDP_PPPRastersToNational (arg0_options) {
		let options = (arg0_options) ? arg0_options : {};
		let hyde_years = landuse_HYDE.sorted_hyde_years;
		...
		console.log(`[GDP_PPP_pc] Running fast sequential pre-pass for Step H thresholds...`);
		for (let i = 0; i < hyde_years.length; i++) {
		...
		await GeoPNG.processTimeseriesParallel({
			concurrency: options.concurrency || 8,
			items: hyde_years,
			name: "GDP_PPP_pc H_scaleGDP_PPPRastersToNational",
			handler: async (year) => {
				if (!thresholds[year]) return null;
				let threshold = thresholds[year].threshold;
				let stats = thresholds[year].stats;
				let local_input_file_path = `${this.intermediate_gdp_ppp_interpolated}GDP_PPP_${year}.png`;
				let local_input_raster = GeoPNG.loadNumberRasterImage(local_input_file_path, { format: "float32" });
				let local_popc_file_path = `${population_Stadester.input_popc_folder}stadester_population_${year}.png`;
				let local_popc_raster = GeoPNG.loadNumberRasterImage(local_popc_file_path, { format: "float32" });
				let local_output_file = `${this.intermediate_gdp_ppp_scaled_to_national}GDP_PPP_${year}.png`;
				...
				console.log(`- Finished GDP PPP ${year}. Healthy max PC: ${thresholds[year].max} (Threshold: ${threshold})`);
			}
		});
	}
	
	static async I_recalculateGDP_PPP_pcRasters (arg0_options) {
		let options = (arg0_options) ? arg0_options : {};
		let hyde_years = landuse_HYDE.sorted_hyde_years;
		let previous_max_pc = 0;
		let thresholds = {};
		
		console.log(`[GDP_PPP_pc] Running fast sequential pre-pass for Step I thresholds...`);
		for (let i = 0; i < hyde_years.length; i++) {
			let current_year = hyde_years[i];
		...
		await GeoPNG.processTimeseriesParallel({
			concurrency: options.concurrency || 8,
			items: hyde_years,
			name: "GDP_PPP_pc I_recalculateGDP_PPP_pcRasters",
			handler: async (year) => {
				if (!thresholds[year]) return null;
				let threshold = thresholds[year].threshold;
				let max = thresholds[year].max;
				let total_file_path = `${this.intermediate_gdp_ppp_scaled_to_national}GDP_PPP_${year}.png`;
				let popc_file_path = `${population_Stadester.input_popc_folder}stadester_population_${year}.png`;
				let output_file_path = `${this.output_gdp_ppp_pc_folder}GDP_PPP_pc_${year}.png`;
				...
				console.log(`- Finalized GDP PPP pc for ${year}. Healthy max: ${max}`);
			}
		});
	}
	...
	static async processRasters (arg0_options) {
		...
		if (!options.exclude.includes("C")) await this.C_generateOLS_GDP_PPP_pcRasters(options);
		...
		if (!options.exclude.includes("F")) await this.F_scaleGDP_PPPRastersToGlobal(options);
