global.GDP_PPP_pc = class {
	...
	static async A_generateGDP_pcRasters (arg0_options) {
		...
		return GeoPNG.processTimeseriesParallel({
			...
			items: target_years,
			name: "GDP_PPP_pc A_generateGDP_pcRasters",
			task_generator: (year) => ({
				clamp_alpha: 200000,
				format: "float32",
				input_path_1: `${GDP_PPP.intermediate_scaled_to_global}GDP_PPP_${year}.png`,
				...
				op: "divide",
				output_path: `${this.input_gdp_pc_folder}GDP_PPP_pc_${year}.png`,
				type: "raster_operation"
			}),
			handler: async (year) => {
				let local_gdp_file_path = `${GDP_PPP.intermediate_scaled_to_global}GDP_PPP_${year}.png`;
				let local_output_file_path = `${this.input_gdp_pc_folder}GDP_PPP_pc_${year}.png`;
