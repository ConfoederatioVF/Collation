...
{
  if (!global.UNModelLifeTables)
    /**
     * UNModelLifeTables implements the United Nations Model Life Tables framework (1955/1982)
     * covering 5 demographic families (General, Latin American, Chilean, South Asian, Far Eastern).
     * Loads and interpolates directly from official UN Model Life Tables source data (un_model_life_tables.csv).
     *
     * @class UNModelLifeTables
     */
    global.UNModelLifeTables = class {
      static key = "un_model_life_tables";
      static name = "UN Model Life Tables";
      static family = "Life Tables";
      static source_csv_path = path.join(__dirname, "un_model_life_tables.csv");
      ...
      /**
       * Loads and indexes official UN Model Life Tables CSV data.
       *
      ...
      static loadSourceTables () {
        ...
        let csv_path = (fs.existsSync(this.source_csv_path)) ?
          ...
          path.join(h3, "age_sex_counterfactuals/1.1.un_model_life_tables/un_model_life_tables.csv");
        let lines = [];
        let raw_txt = "";
        let table_tree = {};
      ...
      /**
       * Trains the UN Model Life Tables mapping from macroeconomic/demographic covariates to life expectancy (e0)
       * and population growth rate (r), calibrated against empirical HMD training pyramids.
       *
      ...
      static train (arg0_dataset, arg1_options) {
        ...
        for (let i = 0; i < N; i++) {
          ...
          let old_f = y[34];
          let old_m = y[35];
          let child_share = infant_f + infant_m;
          let elderly_share = old_f + old_m;

          let proxy_e0 = 32.0 + 38.0*Math.max(0, Math.min(1.0, (elderly_share*5.0)/(child_share*2.0 + elderly_share*3.0 + 1e-4)));
          e0_train.push(proxy_e0);

          let youth_share = 0;
          for (let c = 0; c < 10; c++) youth_share += y[c];
          r_train.push((youth_share - 0.28)*0.05);
        }

        let fitLinearModel = function (target_vec, lambda) {
          let weights = new Array(K).fill(0);
          let mean_t = target_vec.reduce((a, b) => a + b, 0)/N;
          let intercept = mean_t;
        ...
        let e0_fit = fitLinearModel(e0_train, 1e-2);
        let r_fit = fitLinearModel(r_train, 1e-2);
        ...
        return {
          ...
          type: "un_model_life_tables"
        };
      }

      /**
       * Predicts 36-cohort age-sex distribution vector from a covariate feature vector,
       * querying person-years lived nLx directly from official UN Model Life Tables.
       *
      ...
      static predict (arg0_features, arg1_model) {
        ...
        e0 = Math.max(20.0, Math.min(75.0, e0));
        r = Math.max(-0.015, Math.min(0.040, r));

        let e0_floor = Math.floor(e0);
        let e0_ceil = Math.min(75, Math.ceil(e0));
        let alpha = (e0_ceil === e0_floor) ? 0 : (e0 - e0_floor);

        //Query official UN General Pattern (fallback to other UN types if needed)
        let un_type = "UN_General";
        let tbl = (table_tree && table_tree[un_type]) ? table_tree[un_type] : null;
